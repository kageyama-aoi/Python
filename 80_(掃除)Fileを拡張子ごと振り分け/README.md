...
- **プレビュー**：実行前に「どのファイルがどこへ動くか／なぜスキップされるか」を一覧表示します（ファイルは移動しません）。
  拡張子別の件数パネル（多い順、クリックで絞り込み）と列ヘッダクリックのソートに対応。
- **プレビュー行の右クリック**：その拡張子を任意のグループへ割り当て／除外拡張子に追加、
  ファイル名を除外に追加ができます。確認後 `config.ini` を保存して自動で再プレビューします。
...
拡張子グループや除外リストは ［設定…］（基本／拡張子グループ／除外 の3タブ）から
リスト操作で編集できます。保存しても `config.ini` の説明コメントは維持されます。
