...
「折り返し確認」ボタンで、レコード長を変えながら冒頭10レコードの折り返しをプレビューできる
（先頭 レコード長×10 バイトしか読まないため巨大ファイルでも一瞬）。
長さが合っていれば列が揃って見え、ズレていれば文字化けと ✗ NG 表示で分かる。
境界チェックはプレビュー中の全レコードに対して行う（先頭だけだと
「末尾の空白が削れただけ」のズレを見逃すため）。「この値を使う」でパネルに反映。
