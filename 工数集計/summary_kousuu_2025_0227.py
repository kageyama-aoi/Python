...
# ファイル名用のラベルを設定
project_label = target_project if target_project else "全案件"
employee_label = target_employee if target_employee else "全社員"
...
def process_timesheet(file_path, input_tr_csv,target_project,temp_file01,sheet_name_detail,sheet_name_processed,sheet_name_monthly_summary):
    ...
    if target_project:
        ...
        df_filtered = df.copy()  # 取引先を指定しない場合はすべてのデータを取得
...
temp_file01 = f'./output/Summary_{project_label}_{employee_label}_{now:%Y%m%d_%H%M%S}_{random_number}.xlsx'
temp_file02 = './output/temp_file1.xlsx'
final_output = './output/temp2_'+project_label+'_'+employee_label+'aggregate_results.xlsx'  
